...
//   * AND has a .docx attachment.
//   * EXCLUDE admin/thread mail: "Include Mr" subjects, and RE:/FW: replies.
...
export const startsWithDMR = (s) => String(s || '').trim().toLowerCase().startsWith('daily maintenance report')
...
export function classifyMessage(data) {
  ...
  if (!startsWithDMR(subject)) {
    return { ...base, status: 'excluded', reason: isReply(subject) ? 'reply/thread, not a DMR title' : 'subject does not start with "Daily Maintenance Report"' }
  }
  if (!docx.length) {
